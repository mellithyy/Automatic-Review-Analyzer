...
def perceptron(feature_matrix, labels, T):
    ...
    theta = np.array(np.zeros(len(feature_matrix[0])))
    theta_0 = 0
...
def average_perceptron(feature_matrix, labels, T):
    ...
    theta = np.array(np.zeros(len(feature_matrix[0])))
    theta_0 = 0
    final_theta = np.array(np.zeros(len(feature_matrix[0])))
    final_theta_0 = 0
    ...
    for t in range(T):
        for i in get_order(feature_matrix.shape[0]):
            ...
            if labels[i] * (np.dot(theta, feature_matrix[i]) + theta_0) <= 0:
                theta = theta + labels[i] * feature_matrix[i]
                theta_0 = theta_0 + labels[i]
            final_theta = final_theta + theta
            final_theta_0 = final_theta_0 + theta_0

    avg_theta = final_theta / (len(labels) * T)
    avg_theta_0 = final_theta_0 / (len(labels) * T)
